...
savdebt_ts <- as.tibble(cbind(fred_debt_train[1:dlen - 1],
                              fred_savings_train[2:dlen])) %>% 
  rename(debt = V1,
         savings = V2) %>% 
  ts(start = 1980, frequency = 4)
...
savdebt_fit <- auto.arima(savdebt_ts[, 2],
                          xreg = savdebt_ts[, 1],
                          max.P = 0,               # Limiting to P = 0 and Q = 0
                          max.Q = 0)               # because sma1 and sar1 coeffs are NSS on coeftest()
savdebt_fit
