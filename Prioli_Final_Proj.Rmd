...
**Table 3.  Predicted and Observed Values, Disposable Income, Q1 through Q3**
...
savdebt_fit <- auto.arima(dsavings_train,         # auto.arima() suggests ARIMA(2,0,0)(2,0,1)[4]
                          xreg = ddebt_train)#,     # but the ar2 coefficient is nonsignificant;
                          #max.p = 1)              # using max.p = 1 to omit ar(2) term
savdebt_fit
coeftest(savdebt_fit)
```


**Figure `r i`.  Residual Fit, Regression of Differenced Savings on Differenced Debt**

```{r f14_forecast_disp_plot, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 4}
checkresiduals(savdebt_fit)
i <- i + 1
```
