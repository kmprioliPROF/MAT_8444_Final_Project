...
All analyses were performed in R (R v. 3.5.1, R Foundation for Statistical Computing, Vienna, Austria) using packages `forecast` (@pkg_forecast) and `astsa` (@pkg_astsa) for specialized time series calculations and plots.
...
<!-- ```{r sarima_seas, warning = FALSE, fig.width = 10, fig.height = 8} -->
<!-- uplim <- 5 -->

<!-- seas_aic_out <- matrix(double((uplim + 1)^2), nrow = uplim + 1, ncol = uplim + 1) -->
<!-- rownames(seas_aic_out) <- paste0("P_", c(0:(uplim))) -->
<!-- colnames(seas_aic_out) <- paste0("Q_", c(0:(uplim))) -->
<!-- seas_aic_out <- as.data.frame(seas_aic_out) -->

<!-- for(P in 0:uplim){ -->
<!--   for(Q in 0:uplim){ -->
<!--     seas_aic_out[P+1, Q+1] <- sarima(fred_disposable, -->
<!--                                      p = 0, d = 0, q = 0, -->
<!--                                      P = P, D = 1, Q = Q, -->
<!--                                      S = 4, details = FALSE)$AICc -->
<!--   } -->
<!-- } -->
...
<!-- aic_out <- matrix(double((uplim + 1)^2), nrow = uplim + 1, ncol = uplim + 1) -->
<!-- rownames(aic_out) <- paste0("p_", c(0:(uplim))) -->
<!-- colnames(aic_out) <- paste0("q_", c(0:(uplim))) -->
<!-- aic_out <- as.data.frame(aic_out) -->

<!-- for(p in 0:uplim){ -->
<!--   for(q in 0:uplim){ -->
<!--     aic_out[p+1, q+1] <- sarima(fred_disposable,  -->
<!--                                 p = p, d = 1, q = q,  -->
<!--                                 P = 4, D = 1, Q = 5, -->
<!--                                 S = 4, details = FALSE)$AICc -->
<!--   } -->
...
<!-- # Finding best model based on minimizing AICc -->
<!-- which(aic_out == min(aic_out)) -->
<!-- ``` -->
