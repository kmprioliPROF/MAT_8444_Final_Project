...
# ddebt_train <- as.tibble(diff(fred_debt_train)) %>% rename(ddebt = debt) %>% ts(start = 1980.25, frequency = 4)
# dsavings_train <- as.tibble(diff(fred_savings_train)) %>% rename(dsavings = savings) %>% ts(start = 1980.25, frequency = 4)
# dlen <- length(ddebt_train)
# 
# savdebt_fit <- auto.arima(dsavings_train[2:dlen],
#                           xreg = ddebt_train[1:dlen - 1])
# savdebt_fit
...
# savdebt_fit <- auto.arima(fred_savings_train[2:dlen],
#                                  xreg = fred_debt_train[1:dlen - 1])
...
```{r f14_forecast_disp_plot, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 4}
...
```{r f15_savdebt_forecast_plot, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 4}
